...
import org.ggp.base.player.gamer.statemachine.strategic.fixedDepth.MonteCarloTreeSearchHeuristicGamer
import org.ggp.base.util.gdl.grammar.GdlConstant
import org.ggp.base.util.gdl.grammar.GdlPool
import org.ggp.base.util.statemachine.Move

class MonteCarloTreeSearchHeuristicGamerSpec extends GamerSpecification {

	def setup() {
		ruleSheet = ( new AlquerqueGameDescription()).ruleSheet
		gamer = new MonteCarloTreeSearchHeuristicGamer(2)
		roleNotInControl = GdlPool.getConstant("black")
	}

	def "Selected move for Alquerque is one of five when level limit is 5"() {
		given:
		gamer = new MonteCarloTreeSearchHeuristicGamer(7)
	...
	def "Selected move for Alquerque is one of five when level limit is 4"() {
		given:
		gamer = new MonteCarloTreeSearchHeuristicGamer(5)
		GdlConstant RED_PLAYER = GdlPool.getConstant("red")
		long timeout = System.currentTimeMillis() + 20000
	...
	def "Selected move for Alquerque is one of five when level limit is 3"() {
		given:
		gamer = new MonteCarloTreeSearchHeuristicGamer(3)
		GdlConstant RED_PLAYER = GdlPool.getConstant("red")
		long timeout = System.currentTimeMillis() + 20000
